 ...
 * @param {number} options.recoverTime delay in seconds before retrying the downed job server
 */
 ...
 * @param {string} payload opaque data that is given to the function as an argument
 ...
 * @param {string} options.encoding encoding to use
...
Client.prototype.submitJob = function(name, payload, options) {
    ...
    packet = protocol.encodePacket(job.getPacketType(), job.encoding, [job.name, (job.unique ? job.unique : ''), job.payload]);
...
case protocol.PACKET_TYPES.WORK_WARNING:
        job.emit(packetType == protocol.PACKET_TYPES.WORK_DATA ? 'workData' : 'warning', parsedPacket[2]); // trigger event
        break;
